...
import type { BallPattern, TTSLanguage, TTSSpeed, TTSMode, TTSFormat, TTSVoiceSource } from '@/types';
import {
  UI_TRANSLATIONS,
...
import { useBallTTS } from '@/hooks';

interface BallTTSPanelProps {
  pattern: BallPattern;
  className?: string;
...
export function BallTTSPanel({
  ...
  // Update completed beads during playback
  useEffect(() => {
    ...
    <div className={`bg-white border rounded-lg shadow-sm ${className}`}>
      ...
      <div className="px-4 py-3 border-b bg-gray-50 rounded-t-lg">
        ...
        <p className="text-xs text-gray-500 mt-1">
          {settings.language === 'uk' ? `Шар ${pattern.diameter} см` :
           settings.language === 'ru' ? `Шар ${pattern.diameter} см` :
           `Ball ${pattern.diameter} cm`}
        </p>
      </div>
...
// Keep FC export for backwards compatibility
export const BallTTSPanelLegacy = BallTTSPanel;
